...
export default function PurchaseOrderDetailsPage() {
    ...
    async function loadRelatedNames() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setWarehouseName(order.warehouseId ? t("common.unknown") : t("common.noData"));
        }
      }
    }
  ...
  function focusAddProducts() {
    document.getElementById("purchase-order-add-products")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  ...
  return <div className="min-h-full bg-slate-100 p-4 sm:p-6"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="mb-5"><p className="text-xs font-bold uppercase tracking-wider text-blue-600">{t("purchasing.purchaseOrders")}</p><h2 className="mt-1 text-xl font-bold text-slate-900">{t("common.actions")}</h2><p className="mt-1 text-sm text-slate-500">{t("purchasing.orderSummary")}</p></div><div className="grid gap-3 md:grid-cols-4">{workflow.map(([number,title,description]) => { const active=isWorkflowActive(number); const addProductsStep=number === "1" && canAddItems; return addProductsStep ? <button key={number} type="button" onClick={focusAddProducts} className={`w-full rounded-2xl border p-4 text-left transition ${active ? "border-blue-300 bg-blue-50 shadow-sm hover:border-blue-400 hover:bg-blue-100" : "border-slate-200 bg-slate-50"}`}><div className="flex items-start gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold ${active ? "bg-blue-600 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200"}`}>{number}</span><div><p className="font-semibold text-slate-900">{title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{description}</p></div></div></button> : <div key={number} className={`rounded-2xl border p-4 transition ${active ? "border-blue-300 bg-blue-50 shadow-sm" : "border-slate-200 bg-slate-50"}`}><div className="flex items-start gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold ${active ? "bg-blue-600 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200"}`}>{number}</span><div><p className="font-semibold text-slate-900">{title}</p><p className="mt-1 text-xs leading-5 text-slate-500">{description}</p></div></div></div>; })}</div><div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4"><PurchaseOrderActions order={currentOrder} /></div></section>
    {canAddItems && <section id="purchase-order-add-products" className="scroll-mt-6 rounded-2xl border border-blue-200 bg-white p-5 shadow-sm sm:p-6"><div className="mb-5 flex items-start gap-3"><div className="rounded-xl bg-blue-50 p-2.5 text-blue-600"><Package className="h-5 w-5" /></div><div><p className="text-xs font-bold uppercase tracking-wider text-blue-600">{t("purchasing.addProduct")}</p><h2 className="mt-1 text-xl font-bold text-slate-900">{t("purchasing.addProducts")}</h2><p className="mt-1 text-sm text-slate-500">{t("purchasing.orderedProducts")}</p></div></div><PurchaseOrderItemForm purchaseOrderId={currentOrder.id} tenantId={currentOrder.tenantId} onAddItem={handleAddItem} /></section>}
