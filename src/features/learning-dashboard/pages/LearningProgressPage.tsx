    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        ...
        <div className="mt-5 space-y-4">
          {mastery.length ? mastery.map((item) => <div key={item.id}><div className="flex items-center justify-between gap-4 text-sm"><span className="font-medium text-slate-800">Objective {item.objectiveId}</span><span className="text-slate-500">{Math.round(item.masteryScore)}% Â· {stateLabel[item.state] ?? item.state}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-900" style={{ width: `${Math.max(0, Math.min(100, item.masteryScore))}%` }} /></div></div>) : <p className="text-sm text-slate-500">Mastery data will appear after you complete learning activities and assessments.</p>}
        </div>
      </section>
      <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        <h2 className="text-lg font-semibold text-slate-900">Assessment history</h2>
        <div className="mt-4 divide-y divide-slate-100">
          {events.length ? events.slice(0, 20).map((event) => <div key={event.id} className="flex items-center justify-between gap-4 py-3 first:pt-0"><div><p className="text-sm font-medium text-slate-800">Objective {event.objectiveId}</p><p className="mt-1 text-xs text-slate-500">{event.attemptId ? `Assessment attempt ${event.attemptId}` : "Mastery update"}</p></div><span className="text-sm text-slate-600">{event.previousScore == null ? "New" : `${Math.round(event.previousScore)}% â†’ ${Math.round(event.newScore)}%`}</span></div>) : <p className="text-sm text-slate-500">No mastery events have been recorded yet.</p>}
        </div>
      </section>
    </div>
  );
