...
function isIncomingMovement(movementType: string): boolean {
  return [
    "INITIAL_STOCK",
    "PURCHASE_RECEIPT",
    "SALE_RETURN",
...
function isOutgoingMovement(movementType: string): boolean {
  return [
    "SALE",
    "PURCHASE_RETURN",
    "TRANSFER_OUT",
...
export function InventoryHistoryPage() {
  const { ledger, loading, error, refresh } = useInventoryLedger();
  ...
  const summary = useMemo(() => {
    ...
    for (const transaction of ledger) {
      ...
      if (isIncomingMovement(transaction.movementType)) {
        totalIn += quantity;
      } else if (isOutgoingMovement(transaction.movementType)) {
        totalOut += quantity;
      }

      transactionValue += quantity * transaction.unitCost;
    }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <p className="text-sm text-gray-500">Movement Value</p>
          <p className="mt-2 text-2xl font-semibold">
            {formatNumber(summary.transactionValue)}
          </p>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Movement</th>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              ...
              {ledger.map((transaction: InventoryLedgerEntry) => {
                const quantity = Math.abs(transaction.quantity);
                const value = quantity * transaction.unitCost;
                const isIncoming = isIncomingMovement(transaction.movementType);
                const isOutgoing = isOutgoingMovement(transaction.movementType);
                ...
                return (
                  <tr key={transaction.id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-4 py-3">
                      ...
                      >
                        {formatMovementType(transaction.movementType)}
                      </span>
                    </td>
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-right text-sm text-gray-700">
                      {formatNumber(transaction.unitCost)}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <p className="max-w-xs truncate font-medium text-gray-900">
                        {transaction.warehouseName ?? "Unknown Warehouse"}
                      </p>
                    </td>

                    <td className="px-4 py-3 text-sm text-gray-600">
                      {transaction.referenceType
                        ? formatMovementType(transaction.referenceType)
                        : "-"}
                    </td>
                  </tr>
                );
