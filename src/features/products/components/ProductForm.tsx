...
export function ProductForm({ defaultValues, onSubmit = () => {}, loading = false }: ProductFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 rounded-xl border bg-white p-6">
      ...
      <div><label className={labelClass}>{t("products.category")}</label><select {...register("categoryId")} className={fieldClass}><option value="">{t("products.category")}</option>{categories.filter((category) => category.status === "active").map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select>{errors.categoryId && <p className={errorClass}>{errors.categoryId.message}</p>}</div>
      <div><label className={labelClass}>{t("products.brand")}</label><select {...register("brandId")} className={fieldClass}><option value="">{t("products.brand")}</option>{brands.filter((brand) => brand.status === "active").map((brand) => <option key={brand.id} value={brand.id}>{brand.name}</option>)}</select>{errors.brandId && <p className={errorClass}>{errors.brandId.message}</p>}</div>
      <div><label className={labelClass}>{t("products.unit")}</label><select {...register("unitId")} className={fieldClass}><option value="">{t("products.unit")}</option>{units.filter((unit) => unit.status === "active").map((unit) => <option key={unit.id} value={unit.id}>{unit.name} ({unit.symbol})</option>)}</select>{errors.unitId && <p className={errorClass}>{errors.unitId.message}</p>}</div>
