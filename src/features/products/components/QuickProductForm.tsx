...
export function QuickProductForm({tenantId,onCreated}:QuickProductFormProps){
  const [name,setName]=useState("");const [sellingPrice,setSellingPrice]=useState("");const [costPrice,setCostPrice]=useState("");const [error,setError]=useState("");
  const createProduct=useCreateProduct(tenantId);
  useEffect(()=>{if(tenantId)useSettingsStore.getState().loadSettings(tenantId);},[tenantId]);
  const currency=useSettingsStore(s=>s.settings?.currency??"THB");
  async function submit(){
    ...
    if(clean.length<2)return setError("Enter a product name.");if(!Number.isFinite(price)||price<0)return setError("Enter a valid selling price.");
    try{const product=await createProduct.mutateAsync({tenantId,storeId,name:clean,sku:makeSku(clean),costPrice:Number.isFinite(cost)&&cost>=0?cost:0,sellingPrice:price,currency,trackInventory:true});
  ...
  return <div className="rounded-xl border bg-white p-5"><div className="mb-4"><h2 className="text-lg font-semibold">Quick Add Product</h2><p className="text-sm text-gray-500">Start selling with only the information you have. Add the full profile later.</p></div>
    <div className="grid gap-3 md:grid-cols-3"><input value={name} onChange={e=>setName(e.target.value)} placeholder="Product name" className="rounded-lg border p-2"/>
