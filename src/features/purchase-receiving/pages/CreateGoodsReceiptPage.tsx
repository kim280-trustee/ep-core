...
export default function CreateGoodsReceiptPage() {
  ...
  async function handleSubmit() {
    ...
    if (!selectedOrder) return setMessage(t("receiving.selectPurchaseOrderRequired"));
    if (!selectedOrder.warehouseId) return setMessage(t("receiving.purchaseOrderWarehouseRequired"));
    if (!selectedItem) return setMessage(t("receiving.selectItemRequired"));
    ...
    if (quantity > remaining) return setMessage(`${t("receiving.onlyRemainingUnits")} ${remaining}.`);
    ...
    try {
      ...
      setMessage(`${receipt.receiptNumber} ${t("receiving.createdSuccessfully")}. ${t("receiving.stockUpdated")}.`);
      setPurchaseOrderId("");
      ...
      setMessage(error instanceof Error ? error.message : t("receiving.unableToReceiveStock"));
    }
  }
    ...
    <div className="min-h-full bg-slate-100 p-4 sm:p-6">
      <div className="mx-auto max-w-5xl space-y-6">
            ...
            <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
              <div className="max-w-3xl">
                <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-slate-300">
                  <ClipboardCheck className="h-3.5 w-3.5" /> {t("navigation.purchasing")}
                </div>
                <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">{t("receiving.createGoodsReceipt")}</h1>
                <p className="mt-3 text-sm leading-6 text-slate-300 sm:text-base">{t("receiving.receiveApprovedOrderDescription")}</p>
              </div>
        ...
        <section className={`rounded-2xl border bg-white p-5 shadow-sm sm:p-6 ${selectedOrder ? "border-blue-200" : "border-slate-200"}`}>
          ...
          {selectedItem && <div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-blue-100 bg-blue-50/70 p-4"><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">{t("receiving.remainingQuantity")}</p><p className="mt-1 text-2xl font-bold text-slate-900">{remainingQuantity}</p><p className="mt-1 text-xs text-slate-500">{t("receiving.unitsOutstanding")}</p></div><div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">{t("receiving.unitCost")}</p><p className="mt-1 text-2xl font-bold text-slate-900">{selectedItem.unitCost}</p><p className="mt-1 text-xs text-slate-500">{t("receiving.unitCostDescription")}</p></div></div>}
        </section>

        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="mb-5"><p className="text-xs font-bold uppercase tracking-wider text-blue-600">3</p><h2 className="mt-1 text-xl font-bold text-slate-900">{t("receiving.quantityToReceive")}</h2><p className="mt-1 text-sm text-slate-500">{t("receiving.quantityToReceiveDescription")}</p></div>
        ...
        {message && <div className={`flex items-start gap-3 rounded-2xl border p-4 shadow-sm ${message.includes(t("receiving.createdSuccessfully")) ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>{message.includes(t("receiving.createdSuccessfully")) ? <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0" /> : <AlertCircle className="mt-0.5 h-5 w-5 shrink-0" />}<p className="text-sm font-medium leading-6">{message}</p></div>}
      </div>
    </div>
  );
}
