...
export function SupplierForm({ defaultValues, onSubmit }: SupplierFormProps) {
  ...
  const paymentTermsLabel = language === "th" ? "เงื่อนไขการชำระเงิน" : "Payment Terms";
  return <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-xl">
    ...
    <input {...register("paymentTerms")} placeholder={paymentTermsLabel} className="border rounded p-2 w-full" />
