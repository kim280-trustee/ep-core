...
interface SupplierFormProps {
  ...
  onSubmit(data: SupplierFormInput): void;
}
...
export function SupplierForm({ defaultValues, onSubmit }: SupplierFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-xl">
      <div>
        <input {...register("name")} placeholder={t("suppliers.supplierName")} className="border rounded p-2 w-full" />
      ...
      <input {...register("taxId")} placeholder="Tax ID" className="border rounded p-2 w-full" />
      <input {...register("paymentTerms")} placeholder="Payment terms e.g. 30 days" className="border rounded p-2 w-full" />
