...
export function EditCustomerPage() {
  ...
  return <div className="min-h-full bg-slate-100 p-4 sm:p-6"><div className="mx-auto max-w-4xl space-y-5"><section className="overflow-hidden rounded-3xl bg-slate-900 px-5 py-6 text-white shadow-sm sm:px-7 sm:py-7"><button type="button" onClick={() => navigate("/customers")} className="mb-5 inline-flex items-center gap-2 text-sm font-medium text-slate-300 transition hover:text-white"><ArrowLeft className="h-4 w-4" />{t("common.back")}</button><div className="flex items-start gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-blue-500/15 text-blue-300"><Pencil className="h-6 w-6" /></div><div className="min-w-0"><h1 className="text-2xl font-bold tracking-tight sm:text-3xl">{t("customers.editCustomer")}</h1><p className="mt-2 text-sm leading-6 text-slate-300">{t("customers.customerManagement")}</p><div className="mt-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-sm font-medium text-slate-200"><UserRound className="h-3.5 w-3.5" />{customer.name}</div></div></div></section><CustomerForm defaultValues={{ name: customer.name, phone: customer.phone, email: customer.email, address: customer.address, customerType: customer.customerType, creditLimit: customer.creditLimit }} onSubmit={handleSubmit} /></div></div>;
}
